...
// ================= 新增功能：文字转粒子坐标 =================
// 1. 获取文字的坐标点 (放在这里确保定义在 numberTenShell 之前)
// ================= 新增功能：通用文字烟花 =================
...
// 2. 动态生成文字烟花配置 (接收 text 参数)
const getTextShell = (text, size = 1) => {
    return {
        shellSize: size,
        spreadSize: 380 + size * 80,
        starLife: 2800,
        starDensity: 1,
        color: COLOR.Gold,
        points: getShapePoints(String(text)), // 动态获取传入文字的形状
        glitter: 'light',
        isWordShell: true
    };
};
...
// 全局计数器，用于控制“10”的循环播放
let launchSequenceCount = 0;
// 控制“10”出现的频率，这里设置为每 8 次烟花出现一次
const NUMBER_TEN_INTERVAL = 1;
// 倒计时文字烟花控制变量（移植自“烟花秀-修改版(2)”，并改为从5开始）
let countdownEnabled = false;
let currentCountdownNum = 5;
let countdownTimer = 0;
let countdownCompleted = false;

window.startCountdownText = function startCountdownText() {
    countdownEnabled = true;
    currentCountdownNum = 5;
    countdownTimer = 0;
...
function updateGlobals(timeStep, lag) {
    ...
    if (store.state.config.autoLaunch) {
        ...
        if (autoLaunchTime <= 0) {
            autoLaunchTime = startSequence() * 0.625;
        }
    }

    // ================= 倒计时文字播放逻辑（来自“烟花秀-修改版(2)”） =================
    if (countdownEnabled && currentCountdownNum > 0) {
        countdownTimer -= timeStep;

        // 当计时器归零，发射下一个数字
        if (countdownTimer <= 0) {
            // 创建并发射当前数字
            const shell = new Shell(getTextShell(currentCountdownNum, shellSizeSelector()));
            // 固定位置发射 (屏幕中心偏上)
            shell.launch(0.5, 0.55);

            // 数字减1
            currentCountdownNum--;

            if (currentCountdownNum === 0 && !countdownCompleted) {
                countdownCompleted = true;
                countdownEnabled = false;
                window.countdownTextCompleted = true;
                setTimeout(() => {
                    window.dispatchEvent(new Event('countdownTextComplete'));
                }, 1600);
            }

            // 重置计时器：1000ms (1秒) * simSpeed
            countdownTimer = 1000 * simSpeed;
        }
    }
    // ============================================================================
}
    ...
    burst(x, y) {
        ...
        if (this.points) {
            ...
            this.points.forEach(p => {
                ...
                // 【核心修改】：调整文字烟花的扩散速度
                // 原来是 speed * 1.8。
                // 现在为了不受全局2倍速影响，我们除以 simSpeed (此时为2)，
                // 这样文字就会以"正常速度"慢慢炸开，而背景烟花依然是快节奏。
                const shapeSpeed = (speed / simSpeed) * 1.5;
            ...
            // 2. 伴随圆形烟花 (也稍微调慢一点，保持协调)
            if (this.isWordShell) {
                const ringColor = COLOR.Red;
                createBurst(this.starCount || 100, (angle, speedMult) => {
                    const star = Star.add(
                        x,
                        y,
                        ringColor,
                        angle,
                        // 同样除以 simSpeed 稍微减速，保持与文字同步
                        speedMult * speed * (1.1 / simSpeed),
                        this.starLife * 0.9
                    );
                    star.sparkFreq = 40;
                    star.sparkLife = 300;
        ...
        // ================= 新增逻辑开始：处理自定义形状(文字) =================
            if (this.points) {
                ...
                this.points.forEach(p => {
                    // 计算速度和角度
                    // p.x 和 p.y 是归一化坐标，直接作为速度向量，这样烟花就会按形状向外扩散扩大
                    const distance = Math.sqrt(p.x * p.x + p.y * p.y);
                    const angle = Math.atan2(p.y, p.x);
                    // 速度倍率，调整这个可以让数字炸开的大小变化
                    const shapeSpeed = speed * 2.0;
                    ...
                    const star = Star.add(
                        ...
                        this.color,
                        angle,
                        distance * shapeSpeed, // 速度与距离中心点的距离成正比，保持形状
                        this.starLife + Math.random() * this.starLife * this.starLifeVariation
                    );
