...
// 只保留必要的 DOM 节点（stageContainer 和 canvasContainer）
const appNodes = {
  stageContainer: document.querySelector('.stage-container'),
  canvasContainer: document.querySelector('.canvas-container')
}

// renderApp：无需同步任何表单元素，仅作占位供 store.subscribe 调用
...
// 直接从 store.state.config 读取，不依赖 DOM 表单
function getConfigFromDOM() {
  return Object.assign({}, store.state.config)
}
...
function init() {
  ...
  // 保持暂停，canvas 渲染循环就绪但不运行
  // 等待外部调用 startFireworks() 才真正启动
  renderApp(store.state)
  configDidUpdate()

  // 初始化完成回调（如果外部设置了 window.onFireworksReady，则调用它）
  if (typeof window.onFireworksReady === 'function') {
    window.onFireworksReady()
  }
}

// 记录当前绑定状态，防止重复绑定
let _fireworksStarted = false

/**
 * 启动烟花
 *
 * @param {object}  options
 * @param {boolean} options.autoLaunch    - 自动循环发射，默认 true
 * @param {boolean} options.clickToLaunch - 点击屏幕发射烟花，默认 true
 * @param {boolean} options.sound         - 开启音效（含发射音、爆炸音），默认 true
 */
function startFireworks({ autoLaunch = true, clickToLaunch = true, sound = true } = {}) {
...
/**
 * 手动发射烟花
 * @param {number} x        水平位置 0~1，0=最左，1=最右，默认 0.5（中央）
 * @param {number} height   垂直高度 0~1，0=最低，1=最高，默认 0.7
 * @param {boolean} sound   是否播放发射音效，默认 true
 */
function launchShell(x = 0.5, height = 0.7, sound = true) {
  if (!sound) {
    const prev = store.state.soundEnabled
    store.state.soundEnabled = false
    const shell = new Shell(shellFromConfig(shellSizeSelector()))
    shell.launch(x, height)
    store.state.soundEnabled = prev
  } else {
    const shell = new Shell(shellFromConfig(shellSizeSelector()))
    shell.launch(x, height)
  }
...
// 注意：pointer 事件不在这里绑定，由 startFireworks() 统一管理
